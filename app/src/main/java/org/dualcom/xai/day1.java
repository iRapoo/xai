...
public class day1 extends Fragment {
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container, final Bundle savedInstanceState) {
        ...
        final View view = inflater.inflate(R.layout.day1, container, false);
        ...
        /*if (DATE.getWeekType() == 0) {
            for (int i = 0; i < 4; i++) {
                if(DATE.getNowTime() == i && DATE.getWeek() == 0 && (dow != 6 && dow != 0))
        ...
        //Windows.alert(context, "Проверка","");

        for(int i = 1; i < 5; i++){
            final String json_t = JSON.getJSON(context, "day0", i + "-0", group);
            final TextView TextView_t = (TextView) view.findViewById(LIST.TOPt(i - 1));

            final String json_b = JSON.getJSON(context, "day0", i+"-1", group);
            final TextView TextView_b = (TextView) view.findViewById(LIST.BOTt(i - 1));
        ...
        /*for(int i = 1; i < 5; i++){
            final String json = JSON.getJSON(context, "day0", i + "-0", group);
            final TextView TextView = (TextView) view.findViewById(LIST.TOPt(i - 1));
            if(json != "0") {
        ...
        for(int i = 1; i < 5; i++){
            final String json = JSON.getJSON(context, "day0", i+"-1", group);
            final TextView TextView = (TextView) view.findViewById(LIST.BOTt(i - 1));
            if(json != "0") {
                TextView.setText(json.split("//")[0]);
                TextView.setOnClickListener(new View.OnClickListener() {
                    @Override
