...
public class day2 extends Fragment {
    ...
    public View onCreateView(final LayoutInflater inflater, final ViewGroup container, final Bundle savedInstanceState) {
        ...
        /*if (DATE.getWeekType() == 0) {
            for (int i = 4; i < 8; i++) {
                if(DATE.getNowTime()+4 == i && DATE.getWeek() == 1)
        ...
        for(int i = 1; i < 5; i++){
            final String json = JSON.getJSON(context, "day1", i + "-0", group);
            final TextView TextView = (TextView) view.findViewById(LIST.TOPt(i+3));
            if(json != "0") {
        ...
        for(int i = 1; i < 5; i++){
            final String json = JSON.getJSON(context, "day1", i+"-1", group);
            final TextView TextView = (TextView) view.findViewById(LIST.BOTt(i+3));
            if(json != "0") {
                TextView.setText(json.split("//")[0]);
                TextView.setOnClickListener(new View.OnClickListener() {
                    @Override
        ...
        for(int i = 1; i < 5; i++){
            final String json_t = JSON.getJSON(context, "day1", i + "-0", group);
            final TextView TextView_t = (TextView) view.findViewById(LIST.TOPt(i+3));

            final String json_b = JSON.getJSON(context, "day1", i+"-1", group);
            final TextView TextView_b = (TextView) view.findViewById(LIST.BOTt(i+3));
            ...
            if(json_t.equals(json_b))
                ((LinearLayout) view.findViewById(LIST.BOT(i+3))).setVisibility(View.GONE);
            else{
                if(DATE.getWeekType() == 0)
                    ((LinearLayout) view.findViewById(LIST.TOP(i + 3))).setBackgroundResource(R.drawable.less_now);
                else
                    ((LinearLayout) view.findViewById(LIST.BOT(i + 3))).setBackgroundResource(R.drawable.less_now);
            }
